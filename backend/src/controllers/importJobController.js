...
export async function previewImportJob(req, res) {
  try {
    ...
    return res.json({ totalLines: result.totalLines, validRows: result.records.length, duplicates: result.duplicates, existingWords: result.existingWords, errors: result.errors })
