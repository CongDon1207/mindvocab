...
type Preview = { totalLines: number; validRows: number; duplicates: string[]; existingWords: string[]; errors: { location?: string; message: string }[]; warnings: { location?: string; message: string }[] }
const headers = 'word | meaning_vi | pos | ipa | note | ex1_en | ex1_vi | ex2_en | ex2_vi | fill_en'
const prompt = `Please communicate with me in Vietnamese. If I have not given you a word list, only ask me to send the list. When I give a list, return only a Markdown table with exactly these columns in this order: ${headers}. Keep all listed words, write concise Vietnamese meanings, use noun/verb/adj/adv/prep/phrase/idiom/other for pos, and leave only ipa, note, or fill_en empty when needed. Make two distinct natural examples and, when provided, a distinct natural fill_en sentence (7-18 words); every English sentence must contain the word exactly once. In note, state common context, collocations, or important usage cautions.`
...
export default function UploadWordsDialog({ open, onOpenChange, folderId, onJobCreated }: Props) {
  ...
  return <Dialog open={open} onOpenChange={close}><DialogContent className="sm:max-w-[680px]"><DialogHeader><DialogTitle>Nhập danh sách từ</DialogTitle></DialogHeader>
    ...
    {preview && <div className="rounded border p-3 text-sm space-y-1"><p>{preview.validRows}/{preview.totalLines} dòng hợp lệ.</p><p>{preview.duplicates.length} từ trùng trong bảng; {preview.existingWords.length} từ đã có trong folder.</p>{preview.warnings.slice(0, 5).map((item, index) => <p className="text-amber-700" key={`warning-${index}`}>{item.location}: {item.message}</p>)}{preview.errors.slice(0, 5).map((item, index) => <p className="text-red-600" key={`error-${index}`}>{item.location}: {item.message}</p>)}</div>}
